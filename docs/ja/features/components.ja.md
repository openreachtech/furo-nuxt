...
> **1.12.0 で非推奨。2.0.0 で削除します。** ここに挙げるコンポーネントと、その背後のコンテキストクラスは、次のメジャーバージョンで
> パッケージから外れます。スタイルシート `0200.base.css` と `0300.gimmick.css` も同時に削除します。
>
> アップグレードの前に、必要なコンポーネントをアプリ側へコピーしてください。パッケージへの隠れた依存はありません。
> 1 つの `.vue` ファイルとそのコンテキストクラスだけで完結しており、コピーした時点からアプリ側の資産になります。
> 詳細は下記の [コンポーネントからの移行](#コンポーネントからの移行) を参照してください。
...
2.0.0 へのアップグレード前に実施してください。いずれの手順もメジャーバージョンを待つ必要はなく、現行リリースのままで実施できます。

1. 使用している `.vue` ファイルを `node_modules/@openreachtech/furo-nuxt/lib/components/` から自身の `components/` ディレクトリへ
   コピーし、対応するコンテキストクラスも `lib/contexts/concretes/` から同様にコピーします。
2. コピーしたコンポーネントの参照先を、パッケージのコンテキストからコピーしたコンテキストへ差し替え、アプリ側の import も
   コピーへ向け直します。`BaseFuroContext` はパッケージに残るため、コピーしたコンテキストはそのまま継承を維持できます。
3. コンポーネントのスタイルも一緒に移します。パッケージ側では `@layer furo` に宣言されているので、アプリ側では `@layer app` に
   置いてください。自身のルールが優先される状態を保てます。
4. `nuxt.config.js` の `css` から `0300.gimmick.css` を外します。本番で意味を持つルールは `body:has(dialog[open])` だけで、
   `<dialog>` が開いている間ページのスクロールを固定します。必要であればこの 1 ルールだけアプリ側へコピーしてください。
5. `nuxt.config.js` の `css` から `0200.base.css` を外すのは最後にします。全ページのネイティブタグを再スタイルしているためです。
   外すと、`p + p` `section + section` `h1 + *` が付けていた margin、すべての `<input>` の padding、すべての `<h1>` の
   `capitalize`、`<header>` と `<nav>` の `z-index` が失われます。デザイン上必要なものだけを、自身のクラスに限定して戻してください。

`0000.furo.css`、`0010.variables-palette-color-scale.css`、`0020.variables-z-index.css`、`0100.reset.css` はパッケージに残ります。
これまでどおり import してください。
